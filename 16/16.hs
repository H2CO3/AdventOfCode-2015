...
filterAunts :: [Aunt] -> Aunt -> Bool -> [Aunt]
filterAunts aunts target useExact = filter candidate aunts
...
main = do
	...
	let auntIndices = (candidateIndices sues) . (filterAunts sues sue)
	print $ map auntIndices $ reverse (constrs :: [Bool]) -- [True, False]
